...
5. Create the draft GitHub release with `Scripts/release/publish.sh VERSION UPDATE_ZIP APPCAST SOURCE_COMMIT_FILE`. The script verifies the recorded commit locally and on GitHub, creates the tag at that exact commit even if `main` has advanced, and deliberately stops at a draft.
